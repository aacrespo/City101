# Workflow: Agent Team Architectural Modeling (v3)
...
v2 added output strategy — how the team's work becomes a reproducible artifact.
v3 adds coordination infrastructure from Ruflo study: health-check pings, coordination log, weighted authority, team cap, and "lead never builds" rule.
...
## Team Constraints (NEW in v3)

### Team size cap
**Maximum 8 agents total, 2–3 modelers.** Lock 05 proved that more agents = more coordination overhead, diminishing returns. Prefer fewer modelers building carefully over many modelers building fast.

### Lead never builds
The lead (coordinator) **does not create geometry**. Their job is: assign tasks, run health checks, review proposals, resolve disputes, update the coordination log. The moment a lead starts building, they lose the overview that makes coordination possible.
...
### Coordination Log (NEW in v3)

Create before agents start: `output/city101_hub/[lock_id]/coordination_log.md`

This is the team's **live shared memory** during the build. Every agent reads it at the start of each round. Every agent appends decisions and interface alerts as they build. The lead reviews it at each phase gate.
...
Record every geometry change, offset, or decision that affects other agents:
- [HH:MM] [agent] Decision: [what + why + dimensions]
  Example: [structure] Removed Col_L0_0_0 for entrance clearance (3.5m required).
  Added transfer beam Col_L0_-1_0 to Col_L0_1_0, depth 0.8m, centered at z=3.2.

## Interface Alerts (issues between systems)
Flag problems that another agent needs to know about:
- [agent_A] → [agent_B]: [description of misalignment + specific dimensions]

## Open Questions (unresolved)
Coordination decisions that need lead approval:
- [agent] [question + what's blocking]

## Round Summary
After each build round, lead documents:
- Round N: [agents], [object count], [issues resolved/deferred], [next focus]
```

**When agents read this**: At round start (full log) and after each agent finishes (append their decisions).
**When lead updates this**: After every phase gate and round discussion.
...
### Step 4 — Health-Check Ping (NEW in v3, lead, mandatory)

Before proceeding to Phase 2, verify each Phase 1 agent's output is real:

```
FOR EACH AGENT REPORTING "DONE":
1. Query Rhino: count objects on agent's assigned layers
2. Compare to Bill of Objects — expected vs. actual count
3. If count = 0 or < 50% expected:
   → FLAG: geometry missing or failed
   → DO NOT PROCEED to Phase 2
   → SendMessage to agent: "Object count mismatch. Verify build succeeded."
4. If count ≥ 85% expected:
   → MARK: agent healthy, layer verified
   → PROCEED: agent eligible for Phase 2
5. Document results in coordination_log.md:
   Agent: [name] | Expected: [N] | Actual: [N] | Status: PASS / RETRY
```

This catches silent failures (script errors, empty layers) before they cascade into Phase 2.

---

## Phase 2: Detail (parallel, after freeze + health check)
...
### Health-Check Ping (repeat after Phase 2)

Same protocol as Phase 1 Step 4. Run health checks on all detail agents before entering review.
...
Cross-reference all boundary issues against `coordination_log.md` — if an issue was already logged and resolved, verify the resolution is present in the model. If it's a NEW bilateral issue not yet in the log, the involved agents add it immediately.
...
Lessons from Lock 05 + Ruflo study patterns. The lead coordinates the team — these tips are for whoever is running the session.
...
### Authority Hierarchy for Interface Disputes (NEW in v3)

When two agents disagree about a shared interface dimension or decision, use this hierarchy instead of ad-hoc judgment:

**STRUCTURAL GRID AUTHORITY:**
→ Structure agent is authoritative on column positions, slab edges, beam depths, level datums.
→ If any other agent's geometry conflicts with structure's grid: the other agent adapts.
→ Exception: if structure's grid violates code (e.g., column too close to ramp landing), flag for human decision.

**ENVELOPE AUTHORITY:**
→ Shell agent is authoritative on wall thickness, insulation layers, exterior cladding.
→ If shell conflicts with structure (e.g., wall footprint overlaps a column):
  → Structure's grid is authoritative. Shell adapts — offset wall, thicken wall, or wrap column.
→ If shell conflicts with windows (e.g., window frame doesn't fit wall void):
  → Shell adjusts the void to fit windows, not the other way around.

**OPENING AUTHORITY (Windows, Doors):**
→ Windows agent is authoritative for glass size and frame dimensions.
→ Shell agent is authoritative for wall void dimensions.
→ If opening doesn't fit void: shell widens void or shortens opening (not windows reducing glass).
→ Exception: if widening void compromises a column or overstresses the lintel, escalate to lead.

**CODE COMPLIANCE AUTHORITY:**
→ The code is authoritative. No agent overrides SIA/VKF/Swiss building code.
→ If code conflicts with design intent (e.g., railing must be 1.07m, but designer wants 0.9m for proportion):
  → Flag for human decision (Andrea/Henna).
  → Build the code-compliant version; document the override request.

**VISUAL/PROPORTIONAL AUTHORITY:**
→ Reviewer is authoritative for Mode B (visual coherence) after Phase 3.
→ If reviewer says proportions are wrong, the responsible agent adjusts.
→ If the agent disagrees with the reviewer: escalate to human.
→ During builds (Phases 1–2), visual criticism is deferred — focus on constraints first.

**Convergence signal:** If 3+ agents flag the same issue independently → CRITICAL, fix immediately in next round. This is the strongest coordination signal you'll get.

**How to use in round discussions:**
When the lead hears a dispute → check the hierarchy → the answer is deterministic. If hierarchy doesn't resolve it (e.g., code vs. code), escalate to human. This turns authority into a predictable system, not a judgment call every time.
...
| Lead builds | Loses overview, becomes bottleneck | **Lead never builds** (v3 rule) |
| Too many agents | Coordination overhead > output | **Cap at 8 total, 2-3 modelers** (v3 rule) |
...
| Skipping health checks | Silent failures cascade | **Health-check ping after each phase** (v3) |
| Respawning for small fixes | Expensive, loses context | SendMessage to existing agents |
| Ad-hoc dispute resolution | Inconsistent decisions, lead fatigue | **Authority hierarchy** (v3) |
...
4. **Coordination log**: path to the shared log file — read before building, append after
5. **Interface rules**: how their geometry meets adjacent systems
6. **Authority hierarchy**: who wins disputes at each interface (v3)
7. **Code checklist**: relevant standards for their system
8. **Helper functions**: `box()`, `named_box()`, `box_pts()` with SITE_ORIGIN offset
9. **Task assignment**: specific objects to build, with names and coordinates
10. **Spec update responsibility**: after building, update the shared spec with what you actually built and any decisions that deviated from the original plan
...
| 2026-03-26 | v3: Ruflo coordination patterns | Added: health-check pings (Phase 1 Step 4 + Phase 2 exit), coordination log (Phase 0 artifact + round updates), weighted authority hierarchy (Coordinator Guide), team cap (max 8), lead never builds rule, convergence signal, updated anti-patterns table |
