# PROMPT: Build the Full A02 Website — "The Archipelago" (v3)

**What changed from v2:** Added comprehensive design language extracted from viz_08_presentation.html. Added Phase 0 (unify existing vizs to the theme). Added QGIS layout styling guidance. Replaced the skeleton design system with a complete reference. The canonical visual identity is now fully specified — every HTML file must match it exactly.

# THE ARCHIPELAGO — Design Language Reference
# Extract from viz_08_presentation.html — THE canonical visual identity
# EVERY file you create or modify MUST follow this spec exactly.

## Philosophy
Dark, cinematic, data-rich. The aesthetic of an observatory at night — instruments glowing,
data precise, the world outside barely visible. Think Bloomberg Terminal meets Dieter Rams.
NOT a tech startup dashboard. NOT a colorful infographic. Quiet authority.

## Fonts (Google Fonts, import ALL THREE in every HTML file)
```
...
|-----|------|--------|-------|
| Display headings (h1, h2) | Instrument Serif | 400 | Never bold. Letter-spacing: -0.02em. Line-height: 1.05-1.15 |
| Body text | DM Sans | 300-600 | 17px base. Line-height: 1.7 |
| Data labels, stats, mono | DM Mono | 300-500 | All-caps with letter-spacing: 0.15em for labels |
| Chapter numbers | DM Mono | 300 | 0.7rem, uppercase, letter-spacing: 0.25em |
| Stat card values | DM Mono | 500 | 2.2rem, color: accent gold |

**NEVER use**: Inter, Space Mono, system fonts, or any fallback as primary.
...
```css
:root {
  --bg:          #0a0a0f;     /* Near-black with blue undertone — THE background */
  --bg2:         #111118;     /* Card/container background */
  --bg3:         #1a1a24;     /* Elevated surface (hover states, nested cards) */
  --text:        #e8e6e1;     /* Warm off-white body text */
  --text-dim:    #8a8880;     /* Secondary text, labels, descriptions */
  --text-bright: #ffffff;     /* Headings, emphasis */
  --accent:      #c8a86e;     /* Gold — THE signature color. Stat values, borders, highlights */
  --accent-dim:  #8a7548;     /* Muted gold — chapter numbers, decorative lines */
}
```

### Corridor Segments (ALWAYS these 5, in this order)
```css
  --geneva:  #e74c3c;    /* Red */
  --lacote:  #e67e22;    /* Orange */
  --lausanne:#3498db;    /* Blue */
  --lavaux:  #9b59b6;    /* Purple */
  --riviera: #2ecc71;    /* Green */
```

Segment classification by rail_km from Geneva:
- 0-10 km -> Geneva (#e74c3c)
- 10-35 km -> La Cote (#e67e22)
...
### Semantic Colors
```css
  --red:    #e74c3c;     /* Danger, breaks, failures */
  --orange: #e67e22;     /* Warnings, partial */
  --blue:   #3498db;     /* Information, Zurich, neutral data */
  --purple: #9b59b6;     /* Lavaux, night venues, special */
  --green:  #2ecc71;     /* Success, workable, positive */
  --yellow: #f1c40f;     /* Highlight, attention */
```
...
border: 1px solid rgba(200,168,110,0.1);      /* Subtle gold border */
border-radius: 8px;
padding: 1.5rem;
```
...
```css
background: var(--bg2);
border: 1px solid rgba(255,255,255,0.05);     /* Even subtler */
border-radius: 10px;
...
.stat-card .value {
  ...
  font-size: 2.2rem;
  font-weight: 500;
  color: var(--accent);  /* Gold numbers — THE signature move */
  line-height: 1;
}
.stat-card .label {
  font-size: 0.78rem;
...
### SVG / D3 Text
ALL text in charts must use `font-family: 'DM Mono', monospace`.
Axis labels: fill #8a8880. Data labels: fill #c8a86e.
Grid lines: `stroke: rgba(255,255,255,0.06)`.
Axis lines: `stroke: rgba(255,255,255,0.15)`.
Tooltip: background #1a1a24, border `1px solid #8a7548`, text #e8e6e1.

### Leaflet / Map Styling
Tile layer: CartoDB Dark Matter — `https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png`
Station markers: CircleMarker, fillColor by segment, fillOpacity 0.8, weight 1, stroke color `rgba(255,255,255,0.2)`.
Popup: `background: #111118; color: #e8e6e1; border: 1px solid #8a7548;`
Legend: match card styling (bg2 background, DM Mono labels).

## Animations

### Fade-up on scroll (IntersectionObserver, threshold 0.15)
```css
.chapter {
  opacity: 0;
...
### Chart elements
Animate on first visibility only. D3 transitions: duration 800ms, easeQuadOut.
Bars grow from zero height. Dots scale from 0. Lines draw with stroke-dasharray.

### Hover states
Cards: `border-color: #c8a86e; transform: translateY(-4px);` with `transition: 0.3s ease`.

## Layout Rules
- Max content width: 900px, centered
- Chapter padding: 8rem vertical, 2rem horizontal
- Charts get full 900px width with `overflow-x: auto`
- Images: full width, `border-radius: 10px`, `border: 1px solid rgba(255,255,255,0.05)`
- Dividers: 40px wide, 1px, #8a7548, centered, 6rem vertical margin

## Anti-Patterns (NEVER DO)
- White or light backgrounds anywhere (except Marey diagram iframe)
- Bright saturated fills on large areas (only small accents)
- Bold headings (Instrument Serif is always weight 400)
- Rounded/bubbly UI (keep edges precise, border-radius max 10px)
- Emoji or decorative icons
- Gradient backgrounds on cards
- Drop shadows (use borders instead)
- Generic "dashboard" layouts with sidebar + grid
- Any font other than Instrument Serif / DM Sans / DM Mono
- Blue (#1a1a2e) or navy backgrounds — always use #0a0a0f
- Inter, Space Mono, or system font stacks as primary

---

## PHASE 0: Unify Existing Visualizations to the Archipelago Theme

Before building the website, bring all 10 standalone viz files into visual alignment with the design language above. Each file currently has its own ad-hoc styling (Inter font, #1a1a2e backgrounds, Space Mono, etc.).

### Files to unify (in ~/CLAUDE/City101_ClaudeCode/visualizations/):

| File | Size | Current issues |
|------|------|----------------|
| viz_01_ga_cost.html | 21 KB | Inter font, #1a1a2e bg, Space Mono data |
| viz_02_temporal_pulse.html | 24 KB | Wrong fonts, wrong bg |
| viz_03_diversity.html | 26 KB | Same |
| viz_04_zurich.html | 20 KB | Same |
| viz_05_station_explorer.html | 29 KB | Same |
| viz_06_journey_workability.html | 14 KB | Same |
| viz_07_narrative_scroll.html | 28 KB | Same — has its own scrollytelling |
| city101_clock_diagram.html | 9 KB | Check theme alignment |
| city101_marey_diagram.html | 13 KB | KEEP light theme (intentional contrast) |
| city101_spacetime_diagram.html | 11 KB | Check theme alignment |

### For each file (except Marey):
1. Replace font imports with the three Google Fonts (Instrument Serif, DM Sans, DM Mono)
2. Replace background colors: any `#1a1a2e`, `#16213e`, `#0d1117` -> `#0a0a0f`
3. Replace card/container backgrounds -> `#111118`
4. Replace heading fonts -> Instrument Serif 400
5. Replace body fonts -> DM Sans
6. Replace data/mono fonts (Inter, Space Mono) -> DM Mono
7. Replace axis/label colors -> #8a8880 for labels, #c8a86e for values
8. Add the segment color variables if not present
9. Ensure chart containers match the spec above
10. Don't break functionality — only CSS/style changes

### For Marey diagram:
- Leave the light theme. It's a classical transportation diagram and the contrast is intentional.
- ONLY add: consistent segment colors if trains are color-coded, and DM Mono for data labels.

### viz_08_presentation.html:
- This IS the design reference. Don't modify it.
- The website (index.html) will be built from scratch following this exact aesthetic.

### CRITICAL: Don't overwrite originals
Work in `visualizations/site/` — copy files there, modify the copies. Keep originals intact in `visualizations/`.

### QGIS Print Layout Styling Guidance

The 7 QGIS map layouts should feel like they belong to the same project as the HTML visualizations. While QGIS can't perfectly replicate web CSS, apply these principles when fixing/exporting layouts:

**Background**: Set layout background to dark (#0a0a0f or as dark as QGIS allows). Use CartoDB Dark Matter or similar dark basemap.

**Typography**: 
- Titles: Use a serif font (Noto Serif or similar available system serif) — don't use Arial/Helvetica
- Data labels: Use a monospace font (Noto Mono, Courier) at small sizes
- All text: light color (#e8e6e1) on dark background

**Colors**: Use EXACTLY the same 5 segment colors for any corridor data:
- Geneva: #e74c3c, La Cote: #e67e22, Lausanne: #3498db, Lavaux: #9b59b6, Riviera: #2ecc71

**Accent**: Gold (#c8a86e) for key callouts, scale bars, north arrows

**Export**: 150 DPI for web embedding (site/maps/), 300 DPI if print versions needed.

**Credit line**: "Andrea Crespo + Henna Rafik . Studio Huang AR-302(k) . EPFL 2026"
...
**See "THE ARCHIPELAGO — Design Language Reference" at the top of this document.**

Apply it exactly. Every CSS variable, font choice, animation pattern, and component style is specified there. Do NOT improvise or substitute fonts/colors.
