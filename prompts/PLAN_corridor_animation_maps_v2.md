...
These maps reveal corridor behavior patterns — WHERE trains run, WHEN they're busiest,
and WHO is on them — to inform intervention site selection. The specific intervention
type is TBD; these are analytical tools, not illustrations of a decided design.
...
- Source: SBB Passagierfrequenz (already partially in our data as city101_ridership_sbb.csv + city101_service_frequency_v2.csv)
...
**Demographic overlay data (for understanding who uses the corridor):**
...
- Demographic layer toggle: choropleth of communes colored by dominant commute mode or age
...
- Cross-reference with our existing city101_ridership_sbb.csv and city101_service_frequency_v2.csv
...
All raw data outputs go to ~/CLAUDE/City101_ClaudeCode/source/animation/
Agent summaries must list: what was found, what's missing, data quality notes, download URLs for anything not fetched.
...
### Phase 5: Polish
- Apply current design system (see CLAUDE.md — palette, typography, CSS variables). Design system is not locked; follow what exists for consistency.
...
- Raw data saved to ~/CLAUDE/City101_ClaudeCode/source/animation/
- Final HTML outputs to ~/CLAUDE/City101_ClaudeCode/visualizations/
- Agent working outputs go to ~/CLAUDE/City101_ClaudeCode/output/ for verification before promotion
