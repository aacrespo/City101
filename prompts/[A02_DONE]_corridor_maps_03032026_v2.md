...
## Read first
- CLAUDE.md (project conventions, design system, file safety rules)
- PLAN_corridor_animation_maps_v2.md (full spec)
- LEARNINGS.md (known API pitfalls — especially transport.opendata.ch gotchas)

## What to build
Two interactive animated maps of the Geneva–Villeneuve rail corridor (101km, 49 stations).
These reveal corridor behavior patterns — WHERE trains run, WHEN they're busiest, and WHO is on them — to inform intervention site selection. The intervention type is TBD; these are analytical tools.

Project title: "Still on the Line."
...
Respect rate limits: 0.35s minimum between API calls. See LEARNINGS.md for transport.opendata.ch pitfalls (limit parameter can silently flatten data, calendar date filtering required for first/last trains).
...
Cross-reference with existing data in the project:
- `datasets/transit/city101_ridership_sbb.csv` (174 rows, daily pax + commuter index)
- `datasets/transit/city101_service_frequency_v2.csv` (49 rows, trains/hr, IC vs regional)
...
Summary must list: every data source found (even if not downloaded), download URLs, what's missing, quality notes.
...
Summary must list: every data source found, download URLs, what's missing. We need all the demographic data we can get — if there are OFS tables on train ridership by age, income, or nationality, note them even if not downloaded.
...
All agents: save raw data outputs to `~/CLAUDE/City101_ClaudeCode/source/animation/`. Create the directory if needed. Print a comprehensive summary: what was found, what's missing, data quality notes, and download URLs for anything not fetched.
...
**Layer 3 — CHROME (UI controls):** Built entirely with CSS variables from CLAUDE.md design system.
```css
:root {
  --chrome-bg: #0a0a0f;
  --chrome-text: #e8e6e1;
...
```
Design system is not locked — these are current values. If CLAUDE.md has been updated since this prompt was written, use those values instead.
...
Use the existing city101_service_frequency_v2.csv (in `datasets/transit/`) to build a synthetic schedule:
...
## Output locations
- Raw data: `~/CLAUDE/City101_ClaudeCode/source/animation/`
- Agent working outputs: `~/CLAUDE/City101_ClaudeCode/output/` (verify before promoting)
