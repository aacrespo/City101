...
### Systems Engineering Specification v2.0
**Date**: 2026-03-06  
...
**Primary pain**: Andrea and Henna work independently, produce the same datasets, and discover the duplication too late. Six Claude instances and two humans have no shared visibility into what's in progress.
...
├── agent/cairncode/[task]     ← Andrea's Cairn Code agent branches
├── agent/hennacode/[task]     ← Henna's Claude Code agent branches
...
|----------|----------|---------------|-----|
| Claude Code (Andrea) | Yes | Yes (auto-commit) | Direct filesystem |
| Claude Code (Henna) | Yes | Yes (auto-commit) | Direct filesystem |
| Cairn (desktop + MCP) | Yes | Yes (via shell/MCP) | Filesystem access |
| Lumen (desktop + MCP) | Yes | Yes (via shell/MCP) | Filesystem access |
| Meridian (desktop + MCP) | Yes | Yes (via shell/MCP) | Filesystem access |
| Cadence (desktop + MCP) | Yes | Yes (via shell/MCP) | Filesystem access |
...
| Name | Account | Owner | Tier | Best for | Special |
|------|---------|-------|------|----------|---------|
| **Cairn** | Andrea personal | Andrea | Max | Heavy analysis, coordination, strategic thinking | Highest limits |
| **Lumen** | Andrea school | Andrea | Team | QGIS/Rhino MCP, school resources | Lower limits |
| **Cairn Code** | Andrea | Andrea | Autonomous | Batch processing, multi-agent tasks | Auto-commits, filesystem |
| **Meridian** | Henna school | Henna | Team | Henna's primary analysis + MCP | School resources |
| **Cadence** | Henna personal | Henna | Pro | Henna's heavy lifting | Underutilized |
| **Henna's Claude Code** | Henna | Henna | Autonomous | Batch processing, multi-agent tasks | Auto-commits, filesystem |
...
| **Claude Code** (×2) | N/A (IS the agent) | Autonomous multi-agent | Andrea + Henna each have one |
...
|-----------|--------------------|--------------------|
| **Claude Code** | YES — auto-commits throughout | Runs `/session-end` command |
| **VS Code + Claude** | YES — terminal access to git | Human runs `git add -A && git commit && git push` |
| **Desktop + QGIS MCP** | SEMI — can shell out to git via Python | Agent writes session log → human commits |
| **Desktop + Rhino MCP** | SEMI — same as QGIS | Agent writes session log → human commits |
| **claude.ai browser** | NO — read-only access | Human copies text output → commits in next filesystem session |
...
| **A.03** | *(Brief not yet released — likely network strategy / intervention sites)* | W4–W7 | ⏳ Not started |
| **A.04** | *(Brief not yet released — likely architectural proposal at selected sites)* | W5–W7 | ⏳ Not started |
...
## 11. OPEN QUESTIONS (to resolve as briefs arrive)

1. **A.03 / A.04 / A.05 briefs**: When released, integrate milestones into timeline and lockboard task categories.
2. **Blender MCP installation**: Andrea needs to approve. Adds modeling capability but same security surface as Rhino MCP.
3. **VS Code + Claude extension**: Worth setting up as an alternative to Claude Code for sessions where you want the IDE + git in one view?
4. **Cowork**: Claude's desktop automation tool — experimental. Worth monitoring but not building around yet.
5. **Cost of LFS**: Monitor usage. 1GB free. If binary files accumulate fast, may need $5/month data pack.
...
*This document lives at `process/SYSTEM_ARCHITECTURE.md` in the repo. v3 will incorporate lessons from the first 2 weeks of operation (W4–W5).*
