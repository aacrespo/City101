...
Uses `experiments/rhino mcp server/rhino_router_mcp.py`. Every tool call takes a `target` parameter to route to a specific Rhino instance.
...
- Router mode requires a modified rhinomcp plugin that accepts custom ports. Setup instructions are in `experiments/rhino mcp server/SETUP.md`.
